...
case 'gitclone':
if (!args[0]) throw `*[❗𝐈𝐍𝐅𝐎❗] 𝙸𝙽𝙶𝚁𝙴𝚂𝙴 𝚄𝙽 𝙴𝙽𝙻𝙰𝙲𝙴 𝙳𝙴 𝙶𝙸𝚃𝙷𝚄𝙱, 𝙴𝙹𝙴𝙼𝙿𝙻𝙾: ${usedPrefix + command} https://github.com/NKMODS/Zenitsu-Bot-MD*`
...
if (!args[0]) throw `*[❗𝐈𝐍𝐅𝐎❗] 𝙸𝙽𝙶𝚁𝙴𝚂𝙴 𝚄𝙽 𝙴𝙽𝙻𝙰𝙲𝙴 𝚅𝙰𝙻𝙸𝙳𝙾 𝙳𝙴 𝙼𝙴𝙳𝙸𝙰𝙵𝙸𝚁𝙴, 𝙴𝙹𝙴𝙼𝙿𝙻𝙾: ${usedPrefix + command} https://www.mediafire.com/file/pbabuz/𝐙𝐞𝐧𝐭𝐢𝐬𝐮𝐏𝐚𝐠𝐞.zip/file*`
try {
...
`.trim()
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: caption, contextInfo: { externalAdReply: {title: '𝙳𝙴𝚂𝙲𝙰𝚁𝙶𝙰𝚁 𝙳𝙴 𝙼𝙴𝙳𝙸𝙰𝙵𝙸𝚁𝙴', body: 'ʙʏ Zenitsu Bot', thumbnail: imagen1, sourceUrl: 'https://github.com/NKMODS/Zenitsu-Bot-MD' }, mentionedJid: [m.sender]}}}, { quoted: m })
} catch {
...
let json = await res.json()
conn.sendFile(m.chat, json.result.video, 'error.mp4', `_𝐙𝐞𝐧𝐢𝐭𝐬𝐮 𝐁𝐨𝐭 𝐌𝐃_`, m)}
} catch {
m.reply('*[❗𝐈𝐍𝐅𝐎❗] 𝙴𝚁𝚁𝙾𝚁, 𝙿𝙾𝚁 𝙵𝙰𝚅𝙾𝚁 𝚅𝚄𝙴𝙻𝚅𝙰 𝙰 𝙸𝙽𝚃𝙴𝙽𝚃𝙰𝚁𝙻𝙾*')}
break  
...
conn.sendMessage(m.chat, { video: { url: data[data.length - 1] }, caption: '_Zenitsu Bot_', footer: await shortUrl(data[data.length - 1]), buttons }, { quoted: m })
} catch {
try {
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: texto, contextInfo: { externalAdReply: { title: 'Zenitsu Bot', body: null, thumbnail: imagen1, sourceUrl: 'https://github.com/NKMODS/Zenitsu-Bot-MD' }, mentionedJid: [m.sender] }}}, { quoted: m })
...
await conn.relayMessage(m.chat, prep.message, { messageId: prep.key.id, mentions: [m.sender] })
let buttons = [{ buttonText: { displayText: '𝙰𝚄𝙳𝙸𝙾' }, buttonId: `${usedPrefix}tomp3` }]
conn.sendMessage(m.chat, { video: { url: url}, caption: '_Zenitsu Bot_', footer: await shortUrl(url), buttons }, { quoted: m })
} catch {
